...
This project started as a way for me to get hands-on practice with an SQL environment that actually feels like something I might see at a real company, instead of the clean, tiny datasets you typically get from tutorials. I designed the 13-table schema myself to model a real financial institution with customers, branches, employees, multiple account products, beneficiaries, cards, and transactions. To populate it, I used Claude Code to write a Python generator. The generator produces 100,000 customers and 10.6 million transactions across a 25-year span and outputs the data into CSVs. I deliberately made the data messy with inconsistent casing and whitespace, half a dozen phone-number formats, email typos, NULLs, duplicate customers, future-dated records, the occasional negative balance. I wanted to simulate some of the things I might find in a real database that I would need to clean before analysis. From there I built a single-command psql load script that handles all 13 CSVs in FK-dependency order, defers constraint checks for speed, and resets the BIGSERIAL sequences so the database is ready to use afterward. This was all done locally. The next iteration of this project will happen in the cloud.

## Part 2: Data Profiling
Before cleaning, I developed several queries (see data_profiling.sql) to do an intitial analsys to view missing contact info, name hygene, email formats, phone number formats, customer dates of birth innacuracies, account status values, negative balances, value typos, as well as accounts with future open-dates or impossible/suspicious values. This initital query and analysis revealed that there were tens of thousands of errors that needed to be corrected before a detailed analysis could be executed. This also revieled accounts that could be fraudulant, credit accounts that were over the spend limit, future transactions (not pending, but future), and when a loan's maturity date is earlier than its open date. While reviewing the python generator mentioned above, I ensured that this would happen, but these are the kind of queries I would write to look at data inconsistancies before analysis.
...
## Part 5: Dashboard | PowerBI and Tableau
